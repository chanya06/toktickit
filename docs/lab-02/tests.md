...
1. **Unit Tests (`server/tests/lab-02/unit/` & `client/src/tests/lab-02/unit/`)**: Verify ticket number generation formats, file attachment validation rules (size limit, allowed mime types), and date formatters in isolation.
2. **API Integration Tests (`server/tests/lab-02/`)**: Test REST API endpoints using Vitest & Supertest against PostgreSQL test database. Verifies validation rules, ownership enforcement (403/404), attachment upload, download stream, and soft-removal metadata handling.
3. **UI Component Tests (`client/src/tests/lab-02/`)**: Test React components using React Testing Library & Vitest. Verifies component rendering, user interactions, form validation error messages, submitting busy states, and modal dialogs.
4. **UI Style & Responsive Tests (`client/src/tests/lab-02/`)**: Verify CSS Zen Green color token assertions, element accessibility, asterisk markers, and field error placement.
5. **Playwright E2E Tests (`e2e/lab-02/`)**: Full browser end-to-end user journey tests validating Requester selection, ticket creation, attachment upload, My Tickets pagination/filtering, ticket detail inspection, and soft removal.
...
| **UNIT-02** | Unit | BR-10, BR-11 | Attachment file validation logic | Rejects files >5MB or non-image/pdf | `server/tests/lab-02/unit/attachment-validator.test.ts` | Pass |
...
| **UI-01** | UI | AC-02, FR-01 | Development Requester selector screen | Renders active requesters dropdown and persists selection | `client/src/tests/lab-02/RequesterSelect.test.tsx` | Pass |
| **UI-02** | UI | AC-01, FR-03 | Create Ticket form rendering & submission | Submits valid form, shows busy state, renders ticket date | `client/src/tests/lab-02/CreateTicket.test.tsx` | Pass |
| **UI-03** | UI | AC-07, BR-16 | Form submission validation error feedback | Displays inline red error messages, preserves form input | `client/src/tests/lab-02/CreateTicket.test.tsx` | Pass |
| **UI-04** | UI | AC-03, FR-14 | My Tickets table, search bar, & filter controls | Updates list upon typing search term or changing category | `client/src/tests/lab-02/MyTickets.test.tsx` | Pass |
| **UI-05** | UI | AC-09, FR-05 | Ticket Detail read-only layout & Ticket Date | Displays immutable ticket info, Ticket Date, & attachment list | `client/src/tests/lab-02/RequesterTicketDetail.test.tsx` | Pass |
| **UI-06** | UI | AC-05, BR-13 | Soft removal modal prompt & reason validation | Requires reason text before soft-remove confirmation | `client/src/tests/lab-02/AttachmentSection.test.tsx` | Pass |
| **STYLE-01**| Style | Section 7 | Zen Green CSS class assertions & dark text | Verifies `#006B3C` primary classes & contrast | `client/src/tests/lab-02/UIStyleResponsive.test.tsx` | Pass |
| **E2E-01** | E2E | AC-01..10 | Full Requester journey Playwright test | Complete workflow from selector to creation & soft removal | `e2e/lab-02/requester-ticket-flow.spec.ts` | Pass |
...
- [x] **Tablet (834px)**: 2-column form fields, wrap-around filter dropdowns, responsive table layout.
...
All automated tests passed successfully in final main branch build.
